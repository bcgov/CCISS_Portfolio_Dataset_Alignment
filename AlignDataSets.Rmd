...
ss.list <- fread(paste0(cloud_dir, "WNA_SSeries_v12_9.csv"))#%>% filter(!Retire == 'x') %>% dplyr::select(-Retire)
#ss.list <- ss[,.(BGC, SS_NoSpace)]

## 3. All Edatopic (info table from USA_BGC project)
eda <- fread(paste0(cloud_dir, "Edatopic_v12_9.csv")) 
...
feasible <- fread(paste0(cloud_dir, "Feasibility_v12_9.csv"))# %>% filter(!Retire == 'x')
